...
// Insert into comments Docs in Ravendb
$com_count ='1';
$com_rating =strip_tags($_POST['rating']);

$collect =array('@collection' => $product_tb_comment);
$data = array(
    'product_id' => $post_id,
 'comment_id' => $time,
'comment' => $comdesc,
'comment_count' => $com_count,
'comment_rating' => $com_rating,
'timeago' => $timer,
'userid' => $userid_sess,
...
$ch = curl_init();
$url ="$ravendb_database_url/databases/$ravendb_database_name/docs?id=$product_tb_comment/$time";
...
//query the products docs to get comment count
...
$t_comment = $json2['Results'][0]['total_comments'];
$totalcomment_all = $t_comment + 1;
...
'total_comments' => $totalcomment_all,
...
$return_arr = array("comment_rating"=>$com_rating,"comment_count"=>$totalcomment_all,"comdesc"=>$comdesc,"comment_fullname"=>$fullname_sess,"comment_photo"=>$photo_sess,"comment_time"=>$timer);
echo json_encode($return_arr);
