...
//check if User has already like the products post
$ch2 = curl_init();
$url2 ="$ravendb_database_url/databases/$ravendb_database_name/queries";
curl_setopt($ch2, CURLOPT_URL, $url2);
curl_setopt($ch2, CURLOPT_RETURNTRANSFER, 1);
curl_setopt($ch2, CURLOPT_CUSTOMREQUEST, 'POST');
//curl_setopt($ch2, CURLOPT_POSTFIELDS, "{ \n    \"Query\":\"from $product_tb_like where product_id = '$post_id'\" \n}");


curl_setopt($ch2, CURLOPT_POSTFIELDS, "{ \n    \"Query\":\"from $product_tb_like where product_id = '$post_id' AND userid = '$userid_sess'\" \n}");
...
$TotalResults_likes = $TotalResults;
/*
if (curl_errno($ch2)) {
...
if($TotalResults_likes == 1){
	// User already like the item prducts
$return_arr = array("msg"=>"like_already");
echo json_encode($return_arr);
exit();
...
$collect =array('@collection' => $product_tb_like);
$data = array(
    'product_id' => $post_id,
'like_count' => $like,
'timeago' => $timer,
...
$ch = curl_init();
$url ="$ravendb_database_url/databases/$ravendb_database_name/docs?id=$product_tb_like/$time";
...
//query the products docs to get like count
...
$TotalResults = $json2['TotalResults'];
$http_status2 = curl_getinfo($ch2, CURLINFO_HTTP_CODE);
$t_like = $json2['Results'][0]['total_likes'];
$totallike = $t_like + 1;
...
// update like count for Products/Items in RavendB
...
$return_arr = array("like"=>$totallike, "msg"=>"success");
echo json_encode($return_arr);
